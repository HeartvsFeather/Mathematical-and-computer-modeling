...
class Main:
    def game(self):
        ...
        try:
            p = float(input("\nВведите вероятность выпадение орла в формате (0.00): "))
            if p > 1. or p < 0.:
        ...
        print(f"\nПобеды первого если учесть игру без полного разорения: "
              f"{fir_w_count_wfr}")
        ...
        print(f"\nВероятность победы первого если учесть игру без полног разорения: "
              f"{fir_w_count_wfr / n_game}")
        print(f"Вероятность победы второго если учесть игру без полног разорения: "
              f"{sec_w_count_wfr / n_game}")
        print(f"Средняя продолжительность игры: {int(round_count / n)} (раунд) из {n} "
              f"(раунд)")
